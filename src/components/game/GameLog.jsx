...
const GameLog = ({ logs, height, isMobile = false }) => {
    ...
    return (
        <div className={isMobile ? '' : 'glass-panel right-panel-section'} style={{ padding: isMobile ? '0' : '14px 16px', display: 'flex', flexDirection: 'column', height: height === 'auto' ? undefined : (height || '100%'), minHeight: height === 'auto' ? undefined : '200px', flexShrink: height && height !== 'auto' ? 0 : undefined }}>
            ...
            <div style={{ overflowY: height === 'auto' ? 'visible' : 'auto', flex: height === 'auto' ? undefined : 1, display: 'flex', flexDirection: 'column', gap: '4px', minHeight: 0, paddingRight: '4px', marginRight: '-4px' }}>
                {logs.length === 0 && (
                    <span style={{ fontSize: '0.9rem', color: '#ff4d6d', fontStyle: 'italic', fontWeight: 600, textAlign: 'center', padding: '10px 0', display: 'block' }}>No events yet...</span>
                )}
                {logs.map((log) => (
